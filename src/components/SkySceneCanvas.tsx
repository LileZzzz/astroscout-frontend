...
import type { SkyCategory, SkyObject } from "../types/sky101";

const ORBIT_SPEED_MULTIPLIER = 2.25;

type SkySceneCanvasProps = {
  visibleObjects: SkyObject[];
  showLabels: boolean;
  showConstellationLines: boolean;
  onSelect: (obj: SkyObject) => void;
};

export function SkySceneCanvas({
  visibleObjects,
  showLabels,
  ...
  useEffect(() => {
    ...
    const onPointerDown = (evt: MouseEvent) => {
      ...
      if (intersections.length > 0) {
        ...
        if (skyObject) onSelect(skyObject);
      }
    };
  ...
  }, [visibleObjects, showLabels, showConstellationLines, onSelect]);

  return <div ref={mountRef} className="h-[42rem] w-full overflow-hidden sm:h-[52rem]" />;
}
...
function createVisualObject(obj: SkyObject, showLabels: boolean, showConstellationLines: boolean) {
  ...
  if (obj.visualType === "SUN") {
    ...
    const sun = new THREE.Mesh(new THREE.SphereGeometry(obj.radius, 36, 36), sunMaterial);
...
function createLabelSprite(text: string, category: SkyCategory) {
  ...
  const color =
    category === "PLANET" ? "#f7f0d7" : category === "CONSTELLATION" ? "#ffd36e" : "#b9d3ff";
...
function createPlanetTexture(primaryHex: string, accentHex: string) {
  ...
  for (let i = 0; i < 9; i += 1) {
    ...
    ctx.fillStyle = `rgba(${Math.round(mixed.r * 255)}, ${Math.round(mixed.g * 255)}, ${Math.round(
      mixed.b * 255,
    )}, 0.9)`;
